...
# 优化权重：增加 vz (索引 8) 的权重以增强落地阻尼，增加姿态权重 (3, 4) 保持水平
COST_MATRIX_Q = np.diag([1, 1, 80,  40, 40, 1,  2, 2, 80,  1, 1, 1])     # State cost weight matrix
COST_MATRIX_R = np.diag([1e-4] * 12)                                    # Input cost weight matrix
